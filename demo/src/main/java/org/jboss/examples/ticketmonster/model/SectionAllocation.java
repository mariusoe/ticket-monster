 ...
 * 
 * <p>
 * Optimistic locking ensures that two tickets will not be sold within the same row. Adding a member
 * annotated with <code>@Version</code> enables optimistic locking.
 * </p>
 * 
 * <p>
 * The performance and section form the natural id of this entity, and therefore must be unique. JPA
 * requires us to use the class level <code>@Table</code> constraint.
 * </p>
 * 
...
public class SectionAllocation implements Serializable {
	 ...
	 * application, we need to suppress the warnings the java compiler gives us about not using the
	 * field!
	 * </p>
	 */
	...
	/**
	 * <p>
	 * A two dimensional matrix of allocated seats in a section, represented by a 2 dimensional
	 * array.
	 * </p>
	 * 
	 * <p>
	 * A two dimensional array doesn't have a natural RDBMS mapping, so we simply store this a
	 * binary object in the database, an approach which requires no additional mapping logic. Any
	 * analysis of which seats within a section are allocated is done in the business logic, below,
	 * not by the RDBMS.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * <p>
	 * The number of occupied seats in a section. It is updated whenever tickets are sold or
	 * canceled.
	 * </p>
	 *
	 * <p>
	 * This field contains a summary of the information found in the <code>allocated</code> fields,
	 * and it is intended to be used for analytics purposes only.
	 * </p>
	 */
	...
	/**
	 * Post-load callback method initializes the allocation table if it not populated already for
	 * the entity
	 */
	@PostLoad
	void initialize() {
	...
	/**
	 * Allocate the specified number seats within this section for this performance. Optionally
	 * allocate them in a contiguous block.
	 * 
	 * @param seatCount
	 *            the number of seats to allocate
	 * @param contiguous
	 *            whether the seats must be allocated in a contiguous block or not
	 ...
	 * @throws SeatAllocationException
	 *             if the first seat to allocate is more than the number of seats in the row
	 * @throws SeatAllocationException
	 *             if the last seat to allocate is more than the number of seats in the row
	 * @throws SeatAllocationException
