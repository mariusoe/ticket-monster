...
/**
 * <p>
 * A JAX-RS endpoint for handling {@link Booking}s. Inherits the GET methods from
 * {@link BaseEntityService}, and implements additional REST methods.
 * </p>
 *
...
public class BookingService extends BaseEntityService<Booking> {
	...
	public Response createBooking(BookingRequest bookingRequest) {
		try {
			...
			// As we can have a mix of ticket types in a booking, we need to load all of them that
			// are relevant,
			// id
			...
			// Now, we iterate over each ticket that was requested, and organize them by section and
			// category
			// we want to allocate ticket requests that belong to the same section contiguously
			Map<Section, Map<TicketCategory, TicketRequest>> ticketRequestsPerSection = new TreeMap<Section, java.util.Map<TicketCategory, TicketRequest>>(SectionComparator.instance());
			for (TicketRequest ticketRequest : bookingRequest.getTicketRequests()) {
				...
				if (!ticketRequestsPerSection.containsKey(ticketPrice.getSection())) {
					ticketRequestsPerSection.put(ticketPrice.getSection(), new HashMap<TicketCategory, TicketRequest>());
				}
			...
			for (Section section : ticketRequestsPerSection.keySet()) {
				...
				// Compute the total number of tickets required (a ticket category doesn't impact
				// the actual seat!)
	...
	private Map<Long, TicketPrice> loadTicketPrices(Set<Long> priceCategoryIds) {
		List<TicketPrice> ticketPrices = (List<TicketPrice>) getEntityManager().createQuery("select p from TicketPrice p where p.id in :ids", TicketPrice.class).setParameter("ids", priceCategoryIds)
				.getResultList();
		// Now, map them by id
