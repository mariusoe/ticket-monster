...
/**
 * A JAX-RS ExceptionMapper that re-maps RESTEasy ReaderExceptions to ensure that an appropriate
 * response is generated for the mediatype supported by the client.
 *
...
public class ReaderExceptionMapper implements ExceptionMapper<ReaderException> {
	...
	public Response toResponse(ReaderException exception) {
		...
		return Response.status(Response.Status.BAD_REQUEST).entity(responseEntity).type(headers.getMediaType()).build();
	}
}
