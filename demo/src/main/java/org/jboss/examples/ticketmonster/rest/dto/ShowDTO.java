...
public class ShowDTO implements Serializable {
	...
	public ShowDTO(final Show entity) {
		if (entity != null) {
			...
			Iterator<Performance> iterPerformances = entity.getPerformances().iterator();
			...
			Iterator<TicketPrice> iterTicketPrices = entity.getTicketPrices().iterator();
	...
	public Show fromDTO(Show entity, EntityManager em) {
		...
		while (iterPerformances.hasNext()) {
			...
			Iterator<NestedPerformanceDTO> iterDtoPerformances = this.getPerformances().iterator();
		...
		while (iterDtoPerformances.hasNext()) {
			...
			if (found == false) {
				Iterator<Performance> resultIter = em.createQuery("SELECT DISTINCT p FROM Performance p", Performance.class).getResultList().iterator();
		...
		while (iterTicketPrices.hasNext()) {
			...
			Iterator<NestedTicketPriceDTO> iterDtoTicketPrices = this.getTicketPrices().iterator();
		...
		while (iterDtoTicketPrices.hasNext()) {
			...
			if (found == false) {
				Iterator<TicketPrice> resultIter = em.createQuery("SELECT DISTINCT t FROM TicketPrice t", TicketPrice.class).getResultList().iterator();
				while (resultIter.hasNext()) {
	...
	public List<SectionAllocation> findSectionAllocationsByPerformance(Performance performance, EntityManager em) {
		CriteriaQuery<SectionAllocation> criteria = em.getCriteriaBuilder().createQuery(SectionAllocation.class);
		...
		return em.createQuery(criteria.select(from).where(performanceIsSame)).getResultList();
	}
