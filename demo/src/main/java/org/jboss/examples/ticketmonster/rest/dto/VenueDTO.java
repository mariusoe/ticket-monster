...
public class VenueDTO implements Serializable {
	...
	public Venue fromDTO(Venue entity, EntityManager em) {
		...
		if (this.mediaItem != null) {
			entity.setMediaItem(this.mediaItem.fromDTO(entity.getMediaItem(), em));
		}
		...
		while (iterSections.hasNext()) {
			...
			Iterator<NestedSectionDTO> iterDtoSections = this.getSections().iterator();
		...
		while (iterDtoSections.hasNext()) {
			...
			if (found == false) {
				Iterator<Section> resultIter = em.createQuery("SELECT DISTINCT s FROM Section s", Section.class).getResultList().iterator();
	...
	public List<SectionAllocation> findSectionAllocationBySection(Section section, EntityManager em) {
		...
		return em.createQuery(criteria.select(from).where(sectionIsSame)).getResultList();
	}

	public List<TicketPrice> findTicketPricesBySection(Section section, EntityManager em) {
		CriteriaQuery<TicketPrice> criteria = em.getCriteriaBuilder().createQuery(TicketPrice.class);
